...
def test_run_query(reset_db, pause_notification, seed_member, seed_avatar):
    """
    GIVEN a db instance
...
def test_equal_filter():
    ...
    THEN it should return the query result
    """
    s3_filename = f'{str(pytest.member.uuid)}.jpeg'
...
def test_nested_filter(reset_db, pause_notification, seed_member, seed_avatar):
    """
    GIVEN a db instance
    ...
    avatars = db.find(model=Avatar, nested={'member': {'uuid': pytest.member.uuid}})
    assert avatars.total == 1
