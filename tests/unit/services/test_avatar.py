...
# Find
###########
def test_find_avatar(reset_db, pause_notification, seed_member, seed_avatar):
    """
...
def test_avatar_find_expand_member():
    """
    GIVEN 1 avatar instance in the database
    ...
    avatars = avatar_service.find(uuid=pytest.avatar.uuid, expand=['member'])
...
def test_avatar_find_w_pagination(pause_notification):
    ...
    """
    member = services.MemberService().create(status='pending', user_uuid=pytest.other_user_uuid,
    ...
    s3_filename = avatar_service.generate_s3_filename(member_uuid=str(member.uuid))
    avatar = avatar_service.create(s3_filename=s3_filename)
    services.MemberService().apply(instance=member, avatar=avatar)
...
# Create
###########
def test_avatar_create(reset_db, pause_notification, seed_member):
    """
    GIVEN 0 avatar instance in the database
    WHEN the create method is called
    THEN it should return 1 avatar and add 1 avatar instance into the database
    """
    s3_filename = avatar_service.generate_s3_filename(member_uuid=str(pytest.member.uuid))
    avatar = avatar_service.create(s3_filename=s3_filename, member=pytest.member)

    assert avatar.uuid is not None
    assert avatar.s3_filename == s3_filename


def test_avatar_create_w_bad_field(reset_db, pause_notification, seed_member):
    """
    GIVEN 0 avatar instance in the database
    ...
    try:
        s3_filename = avatar_service.generate_s3_filename(member_uuid=str(pytest.member.uuid))
        _ = avatar_service.create(s3_filename=s3_filename, member=pytest.member, junk='junk')
    except ManualException as ex:
        assert ex.code == 500
